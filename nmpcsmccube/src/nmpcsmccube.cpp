...
public:
    NMPCSMCCUBE(): Node("NMPCSMCCUBE"), controlxy_(0.05, 0.1, 0.05, 0.1), xr(5.0), yr(5.0), zr(5.0), psir(0), x(0), y(0), z(0), xd(0), yd(0), zd(0), phi(0), theta(0), psi(0), phid(0), thetad(0), psid(0), w9(0), w10(0), addforce(0), pretime_(this->get_clock()->now()) {
        ...
        ceiling_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>("/addforce", 10, std::bind(&NMPCSMCCUBE::ceilingCallback, this, std::placeholders::_1));
...
private:
    ...
    void ceilingCallback (const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
        if (msg->data.size() != 1) {
            RCLCPP_WARN(rclcpp::get_logger("NMPCSMCCUBE"), "Received incorrect number of side propellers data.");
            return;
        }
    ...
    void stateCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        double U1 = control_variables[0][1] + addforce;
        ...
        std::ofstream outfile1("datanmpcsmccubefailure.txt", std::ios_base::app);
        if (outfile1.is_open()) {
    ...
    rclcpp::Subscription<std_msgs::msg::Float64MultiArray>::SharedPtr ceiling_sub_; 
