...
public:
  Trajectoryprocessor(): Node("point_cloud_to_trajectory"), xs(0), ys(0), zs(0), xds(0), yds(0), zds(0), phis(0), thetas(0), psis(0), phids(0), thetads(0), psids(0) {
    ...
    point_cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>("/lidar/out", 10, std::bind(&Trajectoryprocessor::pointCloudCallback, this, std::placeholders::_1));
  }
...
private:
  ...
  void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
      ...
      double range_wall = (cloud->points.empty()) ? 0.0 : sum_x / cloud->points.size();

      RCLCPP_INFO(this->get_logger(), "Range: %f", range_wall);
      std_msgs::msg::Float64MultiArray trajectory_msg;
      trajectory_msg.data = {0, ys + range_wall - 0.075, 4.0};
